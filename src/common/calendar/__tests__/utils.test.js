...
describe('TimePickerCalendar utils ', () => {
  ...
  const resourceWithMinPeriod = resourceFixture.build({
    min_period: MIN_PERIOD,
  });

  const MAX_PERIOD = '04:00:00';
  const resourceWithMaxPeriod = resourceFixture.build({
    max_period: MAX_PERIOD,
  });

  const selectionLargerThanMinPeriod = {
    ...selection,
    end: new Date('2019-08-16T01:30:00Z'),
  };
  const selectionSmallerThanMaxPeriod = {
    ...selection,
    end: new Date('2019-08-16T03:30:00Z'),
  };

  const selectionMinPeriodExpect = {
    ...selection,
    end: new Date('2019-08-16T01:00:00Z'),
  };
  const selectionMaxPeriodExpect = {
    ...selection,
    end: new Date('2019-08-16T04:00:00Z'),
  };

  describe('getMinPeriodTimeRange', () => {
    test('should return default selected if resource does not have min_period defined', () => {
      const minPeriodTimeRange = calendarUtils.getMinPeriodTimeRange(
        resource,
        selection.start,
        selection.end
      );
    ...
    test('should return default end if end time argument already fulfill the min_period', () => {
      const minPeriod = calendarUtils.getMinPeriodTimeRange(
        resourceWithMinPeriod,
        selectionLargerThanMinPeriod.start,
        selectionLargerThanMinPeriod.end
      );
  ...
  describe('getMaxPeriodTimeRange', () => {
    test('should return default selected if resource does not have max_period defined', () => {
      const maxPeriodTimeRange = calendarUtils.getMaxPeriodTimeRange(
        resource,
        selection.start,
        selection.end
      );
    ...
    test('should return default selected if end time argument already smaller than the max_period', () => {
      const maxPeriod = calendarUtils.getMaxPeriodTimeRange(
        resourceWithMaxPeriod,
        selectionSmallerThanMaxPeriod.start,
        selectionSmallerThanMaxPeriod.end
      );
