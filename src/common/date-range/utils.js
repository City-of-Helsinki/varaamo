...
const isInvalidEndingDay = (range, openingPeriods, startingDays) => {
  const matchingPeriod = findMatchingPeriod(range, openingPeriods);
  if (!matchingPeriod) {
    return false;
  }
  return matchingPeriod.mustEndOnStartDay
    ? startingDays.some(startingDay => moment(startingDay).isSame(moment(range.to), 'day'))
    : false;
};
...
export const validateRange = (range, constraints) => {
  ...
  if (
    isInvalidEndingDay(
      range,
      constraints.openingPeriods,
      constraints.startingDays,
    )
  ) {
    validationErrors.push(DatePickerValidationError.MUST_END_ON_STARTING_DAY);
  }
