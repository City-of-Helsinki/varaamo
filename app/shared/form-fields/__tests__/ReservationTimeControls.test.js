...
describe('shared/form-fields/ReservationTimeControls', () => {
  ...
  describe('render', () => {
    test('renders div.reservation-time-controls', () => {
      expect(getWrapper().find('div.reservation-time-controls')).toHaveLength(
        1
      );
    });
    ...
    test('renders DatePicker with correct props', () => {
      ...
      expect(datePicker.prop('onChange')).toBe(
    ...
    test('renders Select for changing reservation begin time', () => {
      ...
      expect(beginTimeControl.prop('options')).toEqual(
        wrapper.instance().getTimeOptions()
      );
    });
  ...
  describe('getTimeOptions', () => {
    ...
    const optionsToNumber = (options) =>
      options.map(({ value }) => timeToNumber(value));

    test('returns time options using props.period as the duration between times ', () => {
    ...
    test("when constraints.startTime is set, time options before startTime won't be listed", () => {
      ...
      expect(
        optionsToNumber(options).filter(
          (option) => option < timeToNumber(startTime)
        ).length
      ).toEqual(0);
    });
    ...
    test("when constraints.endTime is set, time options after endTime won't be listed", () => {
      ...
      expect(
        optionsToNumber(options).filter(
          (option) => option > timeToNumber(endTime)
        ).length
      ).toEqual(0);
    });
  });
  ...
  describe('handleDateChange', () => {
    ...
    test('calls begin.input.onChange with date updated in begin.input.value', () => {
