...
const AvailabilitySelector = createSelector(resourceSelector, (resource) => {
  if (isEmpty(resource)) {
    return resource;
  }
  const availableTimeByDate = {};
  (resource.openingHours || []).forEach(({ closes, date, opens }) => {
    const openMinutes = moment
      .duration(moment(closes).diff(moment(opens)))
      .asMinutes();
  ...
  return mapValues(availableTimeByDate, (date) => ({
    ...date,
    percentage:
      ((date.openMinutes - date.reservedMinutes) * 100) / date.openMinutes,
  }));
});
